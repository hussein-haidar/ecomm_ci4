...
class M_ongkir_api extends Model
{
    protected $apiKey;
    protected $baseUrl;
    ...
    public function __construct()
    {
        ...
        $this->baseUrl = env('rajaongkir.baseUrl', 'https://rajaongkir.komerce.id/api/v1');
    }
    ...
    public function searchDestination(string $keyword, float $lat = 0, float $lng = 0): array
    {
        ...
        if ($lat != 0 && $lng != 0) {
            ...
            $lngR = round($lng, 2);
            $coordKey = 'search_dest_coord_' . $latR . '_' . $lngR;
            $session->set($coordKey, ['data' => $result['data'], 'time' => time()]);
        }
    ...
    /**
     * Mencocokkan desa/kelurahan hasil reverse geocode (Nominatim) ke data
     * RajaOngkir agar Kecamatan yang sering hilang di OSM bisa terisi.
     */
    ...
    public function calculateCost(int $originId, int $destinationId, int $weightGram, string $courier = 'jne:jnt:sicepat'): array
    {
        ...
        $session->set($cacheKey, ['data' => $result['data'], 'time' => time()]);
